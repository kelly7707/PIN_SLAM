...
class MapVisualizer():
    ...
    def __init__(self, config: Config = None):
        ...
        self.render_odom_trajectory: bool = True # when pgo is on, visualize the odom or not
    ...
    def update(self, scan=None, pose=None, sdf = None, mesh = None, 
               neural_points = None, data_pool = None, pause_now = False):
    ...
    def update_traj(self, cur_pose=None, odom_poses=None, gt_poses=None, pgo_poses=None, pgo_edges=None):
    ...
    def _initialize_visualizer(self):
        ...
        self.vis.create_window(window_name=w_name, width=2560, height=1600) # 1920, 1080
    ...
    def _register_key_callbacks(self):
        ...
        self._register_key_callback(["/"], self._toggle_neural_point_vis_mode) # vis neural point color using feature, ts or certainty
        ...
        self.vis.register_key_callback(265, partial(self._toggle_increase_slice_height)) # up arrow
        self.vis.register_key_callback(264, partial(self._toggle_decrease_slice_height)) # down arrow
    ...
    def _toggle_point_color(self, vis): # actually used to show the source point cloud weight for registration
    ...
    def _toggle_loop_debug(self, vis):
        self.debug_mode = (self.debug_mode + 1) % 3 # 0,1,2 # switch between different debug mode
    ...
    def _toggle_neural_point_vis_mode(self, vis):
        self.neural_points_vis_mode = (self.neural_points_vis_mode + 1) % 5 # 0,1,2,3,4 # switch between different vis mode
    ...
    def _toggle_decrease_mesh_res(self, vis):
        self.mc_res_m = max(self.mc_res_change_interval_m, self.mc_res_m-self.mc_res_change_interval_m)
        print("Current marching cubes voxel size [m]:", f"{self.mc_res_m:.2f}")
    ...
    def _update_geometries(self, scan=None, pose=None, sdf = None, mesh = None, neural_points = None, data_pool = None):
        ...
        if self.render_pointcloud:
            if scan is not None:
                ...
                if self.pc_uniform_color or (self.vis_pc_color and (self.config.color_channel == 0) and (not self.config.semantic_on) and (not self.config.dynamic_filter_on)):
        ...
        if neural_points is not None:
            if self.render_neural_points:
                self.neural_points.points = o3d.utility.Vector3dVector(neural_points.points)
                self.neural_points.colors = o3d.utility.Vector3dVector(neural_points.colors)
            else:
        ...
        if self.render_frame_axis:
            if pose is not None:
                ...
                self.frame_axis = o3d.geometry.TriangleMesh.create_coordinate_frame(size=self.frame_axis_len, origin=np.zeros(3))
    ...
    def _update_traj(self, cur_pose=None, odom_poses=None, gt_poses = None, pgo_poses = None, loop_edges = None):
        ...
        if self.render_trajectory and self.render_gt_trajectory and gt_poses is not None and len(gt_poses) > 1:
