...
def setup_experiment(config: Config, argv = None, debug_mode: bool = False): 
    ...
    if not debug_mode:
        ...
        if config.wandb_vis_on:
            ...
            wandb.run.name = "field_s -  pretrain(ncm&kitti360)- no warmup (freeze/unfreeze 500/50)- " + run_name
            # Set a description for the run
            wandb.run.notes = "|*prior factor, lambda 1e-3 , pretrain(ncm&kitti360) no warmup (freeze/unfreeze 50/500)*| /field_s /| *normal sdf*|  mapping-process_frame-history_idx 0.3*2 around surface* | *6 neighbors*, *calculated warm-up*; no gradient.clip; 3 coord query -> 32 (linear Norm tanh), *(8+3) geo feature+ relative distance -- K|V* -> 32 (layernorm+ tanh); MHA(kv_bias_on + 0.2 dropout, add_bias_kv True); decoder(32->1)|(eikonal loss)gradient_decimation = 1|lr=0.01,1e-3 , no weight decay, adam|*mapper iter=15*, tracker iter =50 | **40 freezing decoder**|reg_lm_lambda= 1e-3 | LiDAR + IMU, PGO"
